...
import { getRequests, deleteConnection } from "../network/Carebitapi";
const ListOfFriendsScreen = ({ navigation }) => {
  ...
  const handleGetRequests = async (tokenData) => {
    const json  = await getRequests(tokenData);
    if (JSON.stringify(backgroundData) !== JSON.stringify(json.connections))
        setBackgroundData(json.connections);

  }

  handleGetRequests(tokenData);
  ...
  const handleDeleteRequest = async (tokenData, rejectID) => {
    deleteConnection(tokenData, rejectID);
